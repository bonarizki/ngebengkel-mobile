    ...
    this.serviceService.RegisterApi(this.FormRegister.value,'register').subscribe(
      data => {
        ...
        if(this.ResponseRegister.status=="error"){       
          this.presentAlert("Pendaftaran user tidak berhasil, silahkan coba lagi.");       
          loading.dismiss();
        }else{
          console.log(this.ResponseRegister.status)
          let message = 'Registration Success, Please Login';
